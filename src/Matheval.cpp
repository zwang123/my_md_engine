...
//  keys.add("optional","VAR",
//    "the names to give each of the arguments in the function."
//    "If you have up to three arguments in your function you can"
//    " use x, y and z to refer to them. "
//    " Otherwise you must use this flag to give your variables names.");
//}
  ...
  evaluator(evaluator_create(const_cast<char*>(func.c_str())))
{
  ...
  //    error("Using more than 3 arguments you "
  //    "should explicitly write their names with VAR");
  ...
  if(!evaluator)
    error("There was some problem in parsing matheval formula "+func);
  ...
  for(int j=0; j<check_count; ++j) {
    ...
    if(!found)
      ...
      error("Variable "+std::string(check_names[j])+
          " cannot be found in your arg");
  }
  ...
  for(size_t i=0; i!=n; ++i) {
    ...
    //evaluator_deriv[i]=evaluator_derivative(
    //  evaluator,const_cast<char*>(var[i].c_str()));
